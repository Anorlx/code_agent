...
def _keyword_fallback(user_input: str, available_tools: dict[str, dict[str, Any]]) -> list[str]:
    ...
    if any(word in text for word in ["fork", "并行调查", "分别分析", "多个模块", "多方向", "并行搜索", "方案比较"]):
        selected.append("fork_tasks")
    if any(word in text for word in ["coordinator", "协调器", "复杂工程", "多阶段", "实施规格", "研究阶段", "综合阶段"]):
        selected.append("coordinator_plan")
