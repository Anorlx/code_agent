...
async def load_memory_context(
    ...
    for relative_path in selected_files:
        try:
            ...
            continue
    selected_memories = "\n\n".join(selected_parts)
    advanced_context = PersonalMemorySystem().context(user_input, session_id=session_id)
    return {
        "index": f"MEMORY.md\n\n{index}",
        "selected_files": selected_files,
        "selected_memories": selected_memories,
        "advanced": advanced_context,
    }
...
def format_memory_context(context: dict[str, Any]) -> str:
    ...
    events = advanced.get("events") if isinstance(advanced, dict) else []
    if cards:
        parts.append("个人长期记忆 Cards（当前版本，带置信度和证据 ID）：")
        parts.append(
            "\n".join(
                f"- [{item.get('memory_id')}] {item.get('category')}: {item.get('fact')} "
                f"(confidence={float(item.get('confidence', 0)):.2f}, evidence={item.get('evidence_event_ids', [])})"
                for item in cards
            )
        )
    if events:
        parts.append("上下文感知历史片段（只作为证据，不直接视为当前事实）：")
        parts.append(
            "\n\n".join(
                f"- {item.get('context_prefix')}\n{item.get('content')}\n"
                f"evidence={item.get('chunk_id')}"
                for item in events
            )
        )
