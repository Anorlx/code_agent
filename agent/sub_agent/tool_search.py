...
15. 如果用户需要回查个人长期记忆、历史对话证据或“我之前说过什么”：选择 search_user_memory。
16. 如果用户要查看或确认候选记忆：选择 list_memory_candidates 或 confirm_memory_candidate。
17. 如果用户要求清理旧工具结果、释放上下文：选择 snip_context。
18. 如果用户需要地图、地址、路线、经纬度：选择 amap 相关 MCP 工具。
19. 如果用户需要联网搜索、网页抓取、新闻、外部资料：选择 tavily 相关 MCP 工具。
20. 如果用户显式输入 /verify、/debug、/simplify 等技能命令，选择对应 skill_* 工具。
21. 如果用户需要验证、调试、简化、批量处理、生成占位文案或把 prompt 变成技能，选择相关 skill_* 工具。
    ...
    return [
        {
            ...
            "exposure": str(info.get("exposure", "lazy")),
...
def _keyword_fallback(user_input: str, available_tools: dict[str, dict[str, Any]]) -> list[str]:
    ...
    if any(word in text for word in ["回忆", "之前说过", "历史记忆", "查记忆", "检索记忆", "search memory"]):
        selected.append("search_user_memory")
    if any(word in text for word in ["候选记忆", "确认记忆", "确认候选", "memory candidate"]):
        selected.append("list_memory_candidates")
        if any(word in text for word in ["确认", "接受", "拒绝", "删除"]):
            selected.append("confirm_memory_candidate")
...
async def select_tools(
    ...
    return [
        ...
        and name not in {"fork_tasks", "coordinator_plan", "tool_search"}
    ]
