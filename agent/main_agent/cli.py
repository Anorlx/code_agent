        ...
        events.append(
            {
                ...
                "message": "Blocking is not supported for session lifecycle hooks; continuing.",
            }
        )
    elif result.action is HookAction.RETRY:
        events.append(
            {
                "type": "hook_retry",
                "event_name": "session.start",
        ...
        events.append(
            {
                ...
                "message": "Blocking is not supported for session lifecycle hooks; shutdown continues.",
            }
        )
    elif result.action is HookAction.RETRY:
        events.append(
...
def _format_token_usage(token_usage: dict[str, Any]) -> str:
    ...
    if kind and kind != "estimate":
        ...
        cache_created = token_usage.get("cache_creation_input_tokens")
        ...
        if cache_created is not None:
            parts.append(f"cache_create={cache_created}")
...
def _execution_mode(event: dict[str, Any]) -> str:
    explicit_mode = _event_field(event, "execution_mode", "")
    if explicit_mode:
...
def _state_suffix(event: dict[str, Any]) -> str:
    ...
    verifier_report = _event_field(event, "verifier_report")
    if isinstance(verifier_report, dict) and verifier_report.get("status"):
        details.append(f"verify {verifier_report.get('status')}")
    prompt_cache = _event_field(event, "prompt_cache_report")
    if isinstance(prompt_cache, dict) and prompt_cache.get("kind"):
        hit = "hit" if prompt_cache.get("stable_cache_hit") else "miss"
        details.append(f"cache {hit}")
...
def _print_event(event: dict[str, Any], ui: TerminalUI) -> None:
    ...
    if event_type == "state":
    ...
    elif event_type == "prompt_cache":
        _close_assistant_line(ui)
        report = event.get("prompt_cache", {})
        hit = "hit" if report.get("stable_cache_hit") else "miss"
        append_only = "append" if report.get("prefix_append_only") else "changed"
        detail = (
            f"{hit} {append_only} "
            f"stable≈{report.get('stable_prefix_tokens')} "
            f"reuse≈{report.get('reused_prefix_tokens')} "
            f"tools={report.get('tool_count')} "
            f"sys={report.get('system_fingerprint')} "
            f"tool={report.get('tools_fingerprint')}"
        )
        if report.get("server_cached_tokens") is not None:
            detail += f" server_cached={report.get('server_cached_tokens')}"
        if report.get("server_cache_creation_input_tokens") is not None:
            detail += f" server_create={report.get('server_cache_creation_input_tokens')}"
        print(ui.event_line("cache", detail, "blue"))
        logger.info("prompt_cache %s", report)
        ...
        failed = [result for result in commands if not result.get("ok")]
        changed_files = event.get("changed_files") or []
        color = "green" if status == "passed" else ("yellow" if status in {"warning", "skipped"} else "red")
        detail = f"{status}: {summary}"
        if commands:
            detail += f" | commands={len(commands)} failed={len(failed)}"
        if changed_files:
            detail += f" | files={len(changed_files)}"
        print(ui.event_line("verify", detail, color))
        for result in failed[:3]:
            command = " ".join(str(part) for part in result.get("command", []))
            output = str(result.get("stderr") or result.get("stdout") or "").strip().splitlines()
            first_line = output[0] if output else "no output"
            print(ui.event_line("verify fail", f"{command}: {first_line}", "red"))
        logger.info(
            "verification status=%s commands=%s failed=%s files=%s summary=%s",
            status,
            len(commands),
            len(failed),
            len(changed_files),
...
def _skills_help_text() -> str:
    ...
    for skill in sorted(skills.values(), key=lambda item: item.name):
        ...
        lines.append(f"/{skill.name}{hint} -- {skill.description}{aliases}")
    return "\n".join(lines)


def _skill_forced_input(user_input: str) -> str | None:
    ...
    if skill is None:
        return None
    return f"/{skill.name} 用户明确调用内置技能 /{skill.name}。参数：{arguments.strip()}"
...
async def _shutdown_background_tasks(
    ...
    *,
    ...
    """Drain memory and summaries, then emit session.end at most once."""
    if history:
    ...
    if session_id is None or (end_state is not None and end_state.emitted):
        return
...
async def chat_loop(max_turns: int, color: bool = False) -> None:
    ...
    while True:
        if pending_user_input is not None:
            ...
            try:
                ...
                await _shutdown_background_tasks(
                    ...
                    hook_manager=hook_manager,
                    session_id=session_record.id,
                    termination_reason="input_interrupted",
                    status="aborted",
        ...
        if user_input == "/skills":
            print(ui.panel(_skills_help_text().splitlines(), strong=True, title="skills"))
            continue
        ...
        if user_input == "/clear":
            ...
            print(ui.welcome(session_record, reader.name, len(tools), max_turns))
            continue
        skill_input = _skill_forced_input(user_input)
