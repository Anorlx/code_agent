...
如果当前任务需要回查过去的个人事实、历史事件或证据，可以使用 search_user_memory；不要假设动态上下文已经包含所有细节。
如果用户要审核敏感候选，可以使用 list_memory_candidates 和 confirm_memory_candidate；未确认候选不得当作正式事实。
记忆只是线索，使用任何记忆前都要结合当前项目状态和记忆证据验证。
"""
...
async def _result_backfill_node(state: AgentGraphState) -> dict[str, Any]:
    ...
    memory_write_tools = {"save_memory", "delete_memory", "prune_memories", "confirm_memory_candidate"}

    def _memory_write_succeeded(result: dict[str, Any]) -> bool:
        if result.get("name") not in memory_write_tools:
            return False
        raw_result = result.get("raw_result")
        return not isinstance(raw_result, dict) or raw_result.get("ok") is not False

    main_agent_saved_memory = any(_memory_write_succeeded(result) for result in tool_results)
    next_state: AgentGraphState = dict(state)
