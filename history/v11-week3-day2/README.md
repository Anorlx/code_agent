...
_一个本地 Python Coding Agent：把 LangGraph 主循环、统一工具系统、上下文管理、长期记忆、MCP 接入和多 Agent 编排组织成清晰的 Agent 工程。_
...
| 工具注册表 | 统一管理内置工具、MCP 工具和未来 skills 工具的 schema、执行函数、并发安全和审查要求 | `agent/tools/registry.py`, `agent/tools/tool`, `agent/tools/mcp`, `agent/tools/skills` |
...
| MCP 接入 | 从 `.mcp.json` 发现 stdio MCP server，把远端 tool 注册成普通工具并复用权限管线 | `agent/tools/mcp`, `.mcp.json`, `mcp-servers/` |
| Fork / Coordinator | Fork 负责并行只读调查，Coordinator 负责 Research + Synthesis 和 scratchpad 实施规格 | `agent/fork`, `agent/Coordinator` |
...
项目根目录使用 `.mcp.json` 管理 MCP server，`agent/tools/mcp/settings.json` 管理权限策略。MCP 工具会被注册成 `mcp__server__tool`，然后和普通工具一样经过 `tool_search -> function schema -> tool_runner -> permission_review -> interactivePrompt`。
...
Week3 Day2 增加了两个编排目录：

- `agent/fork/`：实现 Fork 模式。适合多个互相独立的只读调查任务，例如分别分析 auth、payment、user 模块。Fork worker 继承主上下文，但彼此不通信，结果回到主 Agent 汇总。
- `agent/Coordinator/`：实现 Coordinator 模式的 v1。适合复杂多阶段工程任务，目前执行 Research + Synthesis：并行 research workers 写入 scratchpad，然后 Coordinator 综合成实施规格。

两者遵循文档里的边界：Fork 不递归创建子 Agent；Coordinator 不直接改代码。当前 scratchpad 位于 `.agent_data/coordinator_scratchpad/`，属于本地运行数据，不上传 GitHub。
