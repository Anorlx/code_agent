...
def _fallback_review(tool_name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if tool_name.startswith("skill_"):
        return {"allowed": True, "risk": "low", "reason": "技能工具只生成执行提示；后续真实工具调用仍会单独审查。"}
    ...
    if tool_name == "edit_file":
        return {"allowed": True, "risk": "medium", "reason": "精确替换项目内文件内容，需要谨慎执行。"}
    if tool_name == "todo_write":
        return {"allowed": True, "risk": "medium", "reason": "写入当前会话 todo 状态，会修改本地 .agent_data。"}
    if tool_name == "fork_tasks":
        return {
            "allowed": True,
            "risk": "medium",
            "reason": "Fork 会并行运行只读 worker，适合调查任务，但会消耗额外模型调用。",
        }
    if tool_name == "coordinator_plan":
        return {
            "allowed": True,
            "risk": "medium",
            "reason": "Coordinator 会创建 research workers 并写入本地 scratchpad，但不会直接修改项目代码。",
        }
